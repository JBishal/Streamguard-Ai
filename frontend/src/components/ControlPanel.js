  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 mb-6 shadow-sm flex flex-col md:flex-row items-end gap-5 transform transition-all duration-300 hover:shadow-md relative overflow-hidden group">
      ...
      <div className="flex-1 w-full">
        <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 mt-1">Target Identity string</label>
        <input 
          type="text" 
          value={event}
          onChange={(e) => setEvent(e.target.value)}
          className="w-full bg-slate-50 border border-slate-200 rounded-md px-4 py-2.5 text-[13px] font-mono text-slate-900 focus:outline-none focus:bg-white focus:border-brand-blue focus:ring-1 focus:ring-brand-blue transition-all shadow-inner"
          placeholder="Enter tracking signature..."
        />
      </div>
      ...
      <div className="flex-1 w-full relative">
        ...
        <div className="relative">
          ...
          >
            <option value="football">Node A (Football)</option>
            <option value="cricket">Node B (Cricket)</option>
            <option value="basketball">Node C (Basketball)</option>
            <option value="motorsport">Node D (Motorsports)</option>
            <option value="esports">Node E (eSports)</option>
          </select>
