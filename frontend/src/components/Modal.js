        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <Badge level={data.risk_level || (data.combined_risk_score > 0.7 ? 'High' : data.combined_risk_score > 0.4 ? 'Medium' : 'Low')} />
            </div>
            <p className="font-mono text-[11px] text-gray-500">Cluster ID: {data.cluster_id || 'N/A'} • Score: {(data.risk_score || data.combined_risk_score || 0).toFixed(2)}</p>
          </div>
          <button 
            onClick={onClose}
